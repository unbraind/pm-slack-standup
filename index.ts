...
export interface StandupOptions {
  ...
  trend?: SectionDelta[];
  // Multi-snapshot history (from `--compare <dir>` pointing at a snapshot
  // directory written by `standup export --history-dir`). When present with
...
function assertNoCmdVariableExpansion(argv: readonly string[]): void {
  ...
  if (withQuote !== undefined) {
    throw new CommandError(
      `Refusing to launch pm through cmd.exe: the argument ${JSON.stringify(withQuote)} contains a `
 ...
 *          the spawn must pass.
 */
export function pmLaunchPlan(bin: string, platform: NodeJS.Platform = process.platform): PmLaunch {
  if (platform !== "win32") {
    return {
      command: bin,
      args: (pmArgs) => [...pmArgs],
...
export function resolvePmBin(
  ...
  for (let i = 0; i < 4; i += 1) {
    for (const shim of shims) {
      ...
      if (existsSync(bin)) return pmLaunchPlan(bin, platform);
    }
...
export function buildTextMessage(data: StandupData, opts: StandupOptions): string {
  ...
  if (opts.history && opts.history.length >= 2) {
    ...
    if (historyLine) {
      if (!(opts.trend && opts.trend.length > 0)) lines.push("");
...
export function buildBlockKit(data: StandupData, opts: StandupOptions): { blocks: SlackBlock[]; fallback: string } {
  ...
  if (opts.history && opts.history.length >= 2) {
    const historyLine = renderHistoryLine(opts.history, currentCounts(data));
    if (historyLine) footerElements.push({ type: "mrkdwn", text: historyLine });
  }
  blocks.push({
    type: "context",
 ...
 */
export function renderHistoryLine(history: SnapshotEntry[], current: SectionCounts): string {
  if (history.length < 2) return "";
  const seq = (key: SectionKey): string =>
    [...history.map((s) => s.counts[key] ?? 0), current[key] ?? 0].join("→");
  ...
  activate(api) {
    ...
    const runStandupCommand = async (ctx: CommandHandlerContext) => {
      ...
      if (comparePath) {
        if (isDirectory(comparePath)) {
          ...
          if (history.length === 0) {
            console.error(
              `warning: --compare '${comparePath}' is a directory with no usable standup snapshots; rendering standup without trend deltas.`
            );
          } else {
            opts.trend = computeDeltas(history[history.length - 1].counts, currentCounts(data));
            if (history.length >= 2) opts.history = history;
          }
        } else {
    ...
    if (typeof api.registerService === "function") {
      api.registerService("output_format", (sctx: { command?: string; payload?: unknown }) => {
        const payload = sctx?.payload as Record<string, unknown> | undefined;
        const command = (sctx?.command ?? (payload?.["command"] as string | undefined) ?? "").trim();
    ...
    api.registerExporter(
      ...
      const output: string = fileFormat === "json" ? buildJsonSnapshot() : buildTextMessage(data, exportOpts);
